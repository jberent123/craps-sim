...
def calculate_odds_bet(point, pass_line_bet, bankroll):
    ...
    if point in [4, 10]:
        return pass_line_bet * 3
    elif point in [5, 9]:
        return pass_line_bet * 4
    elif point in [6, 8]:
        return pass_line_bet * 5
    return 0
...
def craps_simulation(bets_per_day, days, initial_bankroll, base_pass_line_bet):
    ...
    current_pass_line_bet = base_pass_line_bet
    ...
    for _ in range(days):
        for _ in range(bets_per_day):
            ...
            if roll in [7, 11]:  # Come-out roll wins
                ...
                current_pass_line_bet *= 2
                largest_pass_line_bet = max(largest_pass_line_bet, current_pass_line_bet)
            else:  # Point is established
                point = roll
                odds_bet = calculate_odds_bet(point, current_pass_line_bet, bankroll - current_pass_line_bet)
                bankroll -= odds_bet
            ...
            if current_pass_line_bet > bankroll:  # Adjust bet if it exceeds the remaining bankroll
                current_pass_line_bet = bankroll
...
def main():
    while True:
        ...
        bets_per_day = 11  # Or prompt the user for this
        days = 3  # Or prompt the user for this
        ...
        for i, (final_bankroll, total_wins, total_losses, total_rolls, largest_bet) in enumerate(simulation_results, start=1):
           # print(f"Simulation Run {i}: | Final Bankroll: ${final_bankroll}, | Wins: {total_wins}, | Losses: {total_losses}, Rolls: {total_rolls}, Largest Bet: ${largest_bet}")
